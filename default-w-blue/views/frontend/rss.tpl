...
{if $release.cover == 1}<imagepath>{$serverroot}covers/movies/{$release.imdbID}-cover.jpg</imagepath>{else}<imagepath>{$serverroot}covers/noposter.jpg</imagepath>{/if}

<posterrow>{$serverroot}details/{$release.guid}</posterrow>

<imdbrating>{$release.rating}</imdbrating>

<movieyear>{$release.year}</movieyear>
<moviesize>{$release.size|fsize_format:"MB"}</moviesize>
<movieplot>{$release.plot}</movieplot>
<movieguid>guid{$release.guid}</movieguid>
<myapi>{$userdata.rsstoken}</myapi>

{if $release.imdbtitle != ""}<mymovietitle>{$release.imdbtitle}</mymovietitle>{else}<mymovietitle>{$release.searchname}</mymovietitle>{/if}
